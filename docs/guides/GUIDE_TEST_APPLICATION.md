...
**Note Linux (desktop)**
- Si vous utilisez Flutter installé via **snap**, le build Linux peut échouer au linkage avec une erreur du type `libsecret-1.so.0: référence indéfinie vers g_task_set_static_name` (mélange de libs snap vs libs système).
- Dans ce cas, privilégiez un smoke test sur **Android** (appareil réel ou émulateur) ou installez le SDK Flutter via l’archive officielle (non-snap) pour éviter ce problème d’outillage.
